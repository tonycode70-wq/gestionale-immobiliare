...
const tenantSchema = z.object({
  ...
  (data) => (data.tipo_soggetto === 'persona_fisica' ? !!(data.nome && data.cognome) : !!data.ragione_sociale),
  {
...
export function TenantEditForm({ tenant, trigger, onSuccess }: TenantEditFormProps) {
  ...
  const form = useForm<TenantFormData>({
    ...
    defaultValues: {
      ...
      indirizzo_residenza: tenant.indirizzo_residenza || '',
      cap_residenza: tenant.cap_residenza || '',
      citta_residenza: tenant.citta_residenza || '',
      provincia_residenza: tenant.provincia_residenza || '',
      email: tenant.email || '',
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Aggiorna i dati del conduttore
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="email" placeholder="mario@email.it" {...field} value={field.value || ''} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Note aggiuntive..." className="resize-none" {...field} value={field.value || ''} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <Button type="submit" disabled={updateTenant.isPending}>
                {updateTenant.isPending ? 'Salvataggio...' : 'Salva Modifiche'}
              </Button>
            </div>
          </form>
        </Form>
